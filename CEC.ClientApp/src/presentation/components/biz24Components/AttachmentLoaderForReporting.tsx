...
const AttachmentLoaderForReporting = (props: any) => {
  const attachments = props.attachments;
  const currentBiznessEventId = props.currentBiznessEventId;
  ...
  const deleteFile = (index: any) => {
    ...
    if (
      allAttachments[index].biznessEventId &&
      allAttachments[index].biznessEventId !== currentBiznessEventId
    ) {
      toast.error(
        'You cannot delete this attachment, as this attachment has been attached from another page/task'
      );
      return false;
    }
          ...
          {attachments.map((perAttachment: any, index: any) => (
            ...
            >
              <div className="flex">
                ...
                <img
                  style={{ objectPosition: 'top', objectFit: 'contain' }} // to show cropped preview image from top side
                  src={
                  ...
                  className="rounded-t-md border-t-2 border-x-2 border-zinc-300 cursor-zoom-in object-contain w-full h-20"
                  alt=""
                  onClick={
...
export default AttachmentLoaderForReporting;
